...
function Header() {
  ...
  const navItems = [
    { name: 'Home', slug: '/home', icon: <Home size={20} />, auth: 'all' },
    { name: 'Explore', slug: '/explore', icon: <Search size={20} />, auth: 'auth' },
    { name: 'Login', slug: '/login', icon: <LogIn size={20} />, auth: 'guest' },
    { name: 'SignUp', slug: '/signup', icon: <LogIn size={20} />, auth: 'guest' },
    { name: 'Profile', slug: '/profile', icon: <User size={20} />, auth: 'auth' },
    { name: 'More', slug: '/more', icon: <MoreHorizontal size={20} />, auth: 'auth' },
  ];
  ...
  return (
    <aside className="hidden sm:flex flex-col w-full sm:w-[250px] p-4 border-r border-gray-200 h-screen fixed left-0 top-0 bg-white justify-between">
      <Container>
        {/* Logo */}
